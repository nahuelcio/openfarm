...
import type { Plan, PlanReview, PlanStatus } from "./types";
import { PlanReviewSchema, PlanSchema } from "./types";
...
export class PlanManager {
	...
	getPlanStatistics(): {
		...
		const totalAnnotations = plans.reduce((total, plan) => {
			return (
				total + this.annotationManager.getAnnotationsForPlan(plan.id).length
			);
		}, 0);
