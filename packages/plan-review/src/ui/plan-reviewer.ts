import type { Plan, Annotation, AnnotationType, TextSelection } from '../types';
import type { PlanReviewerProps } from './types';
...
export class PlanReviewer {
  ...
  private createComponents(): void {
    ...
    this.planDisplay = new PlanDisplay(planContainer, {
      ...
      onTextSelect: (selection: TextSelection) => this.handleTextSelect(selection),
    });
    ...
    this.annotationPanel = new AnnotationPanel(sidebarContainer, {
      ...
      onAnnotationCreate: (type: AnnotationType, content: string, position?: any) => 
  ...
  private handleAnnotationCreate(type: AnnotationType, content: string, position?: any): void {
  ...
  private handleAnnotationUpdate(id: string, updates: Partial<Annotation>): void {
  ...
  private handleApprove(): void {
    if (confirm('Approve this plan? The agent will proceed with implementation.')) {
      this.onPlanApprove();
    }
  }
