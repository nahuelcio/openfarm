...
export class AnnotationPanel {
	...
	constructor(container: HTMLElement, props: AnnotationPanelProps) {
	...
	private createFilters(): HTMLElement {
		...
		const filterButtons = [
			{ key: "all", label: "All", count: this.annotations.length },
			{
				key: "unresolved",
				label: "Unresolved",
				count: this.annotations.filter((a) => !a.resolved).length,
			},
			{
				key: "comments",
				label: "Comments",
				count: this.groupAnnotationsByType().comment?.length || 0,
			},
			{
				key: "changes",
				label: "Changes",
				count: this.getChangeAnnotations().length,
			},
		];
	...
	private getChangeAnnotations(): Annotation[] {
		return this.annotations.filter((a) =>
			["delete", "insert", "replace"].includes(a.type),
		);
	}
