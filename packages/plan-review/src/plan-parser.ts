...
import { PlanSchema, PlanStepSchema } from './types';
...
export class PlanParser {
  static parseMarkdown(markdown: string): Plan {
    ...
    const parsed = this.parseMarkdownLines(lines);
    return this.convertToPlan(parsed);
  }
  ...
  static parseText(text: string): Plan {
    ...
    try {
      ...
      return this.parseSimpleText(text);
    }
  }
  ...
  private static parseMarkdownLines(lines: string[]): ParsedPlan {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.match(/^\d+\.?\s+/) || line.match(/^-\s+/)) {
        if (currentStep) {
          plan.steps.push(this.finalizeStep(currentStep, stepOrder++));
        }
        currentStep = this.parseStepLine(line);
      ...
      if (line.startsWith('Dependencies:') || line.startsWith('Deps:')) {
        if (currentStep) {
          const deps = line.split(':')[1].split(',').map(d => d.trim()).filter(d => d);
  ...
  private static finalizeStep(step: Partial<ParsedStep>, order: number): ParsedStep {
    return {
      order,
