...
export class ProviderRegistry implements IProviderRegistry {
	...
	private async discoverBuiltInProviders(): Promise<void> {
		...
		try {
			const { ClaudeProviderFactory } = await import(
				"../providers/claude-factory"
			);
			const claudeFactory = new ClaudeProviderFactory();
			this.registerProvider(claudeFactory);
		} catch (error) {
			logger.warn("Failed to load ClaudeProvider:", error);
		}

		try {
			const { OpenCodeProviderFactory } = await import(
				"../providers/opencode-factory"
			);
			const opencodeFactory = new OpenCodeProviderFactory();
			this.registerProvider(opencodeFactory);
		} catch (error) {
			logger.warn("Failed to load OpenCodeProvider:", error);
		}
	...
	private async discoverExternalProviders(): Promise<void> {
		...
		// Phase 1 TS-first: Claude and OpenCode are SDK built-ins.
		// Keep this method for future third-party provider discovery only.
	}
