# Project Overview
The OpenFarm SDK provides a unified interface for AI-powered code execution across multiple providers. It allows developers to integrate AI coding assistants into their applications through a consistent, modular API. The SDK handles provider discovery, lazy loading, and standardized execution flows, supporting providers such as Aider, Claude Code, and OpenCode.
...
- `OPENFARM_PROVIDER`: Default provider to use (e.g., `opencode`, `aider`)
...
- **OpenCode**: Supports both cloud API and local CLI modes.