...
import type { WorkspaceProvisionConfig } from "../../../orchestration";
...
export async function executePlatformProvisionPod(
  ...
  if (!services.orchestrator) {
    return err(
      new Error(
        "orchestrator service is required for platform.provision_pod action"
      )
    );
  }
  ...
  await logger(`Provisioning ephemeral workspace for job ${context.jobId}`);

  // Build generic workspace config
  const provisionConfig: WorkspaceProvisionConfig = {
    workspaceId: context.jobId,
    taskId: context.workItem.id,
    ...
    environment: {
      COPILOT_API_URL:
        context.agentConfig?.copilotApiBase ||
        process.env.COPILOT_API_URL ||
        "",
      // Note: K8s-specific options like image, cloneDepth, etc. should be handled by the orchestrator implementation
      // For now, we'll pass them via environment as a workaround until we have proper orchestrator options
      _K8S_IMAGE: validatedConfig.image || "",
      _K8S_CLONE_DEPTH: String(validatedConfig.cloneDepth || 1),
      _K8S_NPM_CACHE: String(validatedConfig.npmCache ?? true),
      _K8S_NAMESPACE: validatedConfig.namespace || "minions-farm",
      _K8S_CPU: validatedConfig.resources?.cpu || "",
      _K8S_MEMORY: validatedConfig.resources?.memory || "",
    },
  };

  const result = await services.orchestrator.provision(provisionConfig);
  ...
  await logger(`Workspace provisioned successfully: ${result.value.id}`);
  return ok(result.value.id); // Return workspace ID
}
...
export async function executePlatformDestroyPod(
  ...
  if (!services.orchestrator) {
    return err(
      new Error(
        "orchestrator service is required for platform.destroy_pod action"
      )
    );
  }
  ...
  if (!podName) {
    await logger(
      "Warning: No podName in context, skipping workspace destruction (may already be destroyed)"
    );
    return ok("No workspace to destroy");
  }

  if (flags.previewMode) {
    await logger(`[Dry Run] Would destroy workspace: ${podName}`);
    return ok("Dry run: Workspace destruction skipped");
  }

  await logger(`Destroying workspace: ${podName}`);
  const result = await services.orchestrator.destroy(podName);
  ...
  await logger(`Workspace destroyed successfully: ${podName}`);
  return ok(`Workspace ${podName} destroyed successfully`);
}
