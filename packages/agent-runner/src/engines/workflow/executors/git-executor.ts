...
export async function executeGitCheckout(
	...
	const { updatedGitConfig } = setupPodAwareGit(context, logger, true);

	await logger(`Checking out branch: ${branch}`);
	const result = await checkoutBranch(updatedGitConfig, branch, {
		defaultBranch,
	});
...
export async function executeGitBranch(
	...
	const { updatedGitConfig } = setupPodAwareGit(context, logger, false);
	...
	// For Pods, create a FileSystem that checks existence via kubectl exec
	const _fsPod: FileSystem | undefined = podName
		? createPodFileSystem(podName, logger)
		: undefined;
	...
	const result = await checkoutBranch(updatedGitConfig, finalBranchName, {
		defaultBranch,
	});
