...
function replaceModeTernaries(
	...
	const complexModeTernaryPattern =
		/\$\{workItem\.mode\?\s*`([^`]+)`\s*:\s*`([^`]+)`\}/g;
	return text.replace(
		complexModeTernaryPattern,
		(match, readOnlyMode, editMode) => {
			return isReadOnly ? readOnlyMode : editMode;
		},
	);
}

function replaceDescriptionTernaries(
	text: string,
	workItem: WorkItemExpressions,
): string {
	const descriptionTernaryPattern =
		/\$\{workItem\.description\?\s*`([^`]+)`\s*:\s*`([^`]+)`\}/g;
	return text.replace(
		descriptionTernaryPattern,
		(match, hasDescription, noDescription) => {
			return workItem.description ? hasDescription : noDescription;
		},
	);
}

function replaceAcceptanceCriteriaTernaries(
	text: string,
	workItem: WorkItemExpressions,
): string {
	const acceptanceTernaryPattern =
		/\$\{workItem\.acceptanceCriteria\?\s*`([^`]+)`\s*:\s*`([^`]+)`\}/g;
	return text.replace(
		acceptanceTernaryPattern,
		(match, hasCriteria, noCriteria) => {
			return workItem.acceptanceCriteria ? hasCriteria : noCriteria;
		},
	);
}

function replaceGenericTernaries(
	text: string,
	workItem: WorkItemExpressions,
): string {
