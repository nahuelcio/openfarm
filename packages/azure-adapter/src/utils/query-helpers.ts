...
import type { WorkItem, AzureConfig, FetchFunction, AzureWiqlResponse } from "../types";
...
export function buildWiqlQuery(workItemTypes?: string[]): string {
  const typeFilter = workItemTypes && workItemTypes.length > 0
...
export async function executeWiqlQuery(
  ...
  if (!wiqlResponse.ok) {
    ...
    return err(new Error(`Azure WIQL query failed: ${wiqlResponse.status} - ${errorBody}`));
  }
  ...
  try {
    ...
    return err(new Error(`Failed to parse WIQL response: ${error instanceof Error ? error.message : String(error)}`));
  }
}
 ...
 */
export function applyPaginationToIds(ids: (string | number)[], pagination?: { offset?: number; limit?: number }): (string | number)[] {
