...
export function getAvailableModels(): string[] {
  try {
    ...
    const output = execSync("curl -s https://models.dev/api.json", {
      encoding: "utf8",
      timeout: 5000,
    ...
    if (data.anthropic?.models) {
      const models = Object.keys(data.anthropic.models).sort((a, b) => {
    ...
    return ["claude-4-5-sonnet", "claude-4-5-haiku", "claude-opus-4-5"];
  } catch (_error) {
