...
function DiffStatusLabel({ status }: { status: FileDiff["status"] }) {
	...
	return (
		<span
			className={cn("text-[10px] font-medium px-1.5 py-0.5 rounded", s.color)}
		>
			{s.text}
		</span>
	);
	...
	return (
		...
		>
			...
			<div className="flex items-center gap-2 shrink-0">
				<span className="text-[11px] font-mono text-agent-active">
					+{diff.linesAdded}
				</span>
				<span className="text-[11px] font-mono text-agent-error">
					-{diff.linesRemoved}
				</span>
	...
	const [selectedFileIndex, setSelectedFileIndex] = useState(() => {
		if (initialFile) {
			const idx = diffs.findIndex(
				(d) =>
					d.path === initialFile ||
					d.filename === initialFile ||
					initialFile.endsWith(d.filename),
			);
			return idx >= 0 ? idx : 0;
		}
		return 0;
								...
								<>
									...
									<div className="flex items-center gap-2 px-4 py-2 border-b border-border/50 bg-secondary/30">
										...
										<div className="ml-auto flex items-center gap-2 text-xs">
											<span className="text-agent-active font-mono">
												+{selectedDiff.linesAdded}
											</span>
											<span className="text-agent-error font-mono">
