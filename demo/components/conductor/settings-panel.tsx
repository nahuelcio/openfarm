...
import type { AppSettings, AgentProvider, ProviderConfig } from "@/lib/store"
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground leading-relaxed">{provider.description}</p>
          </div>
        </div>
        <Switch
      ...
      {provider.connected && (
        <div className="space-y-3 pt-2 border-t border-border/50">
          ...
          <div>
            ...
            {testResult === "success" && (
              <p className="text-[10px] text-agent-active mt-1">Connection successful</p>
            )}
            {testResult === "error" && (
              <p className="text-[10px] text-agent-error mt-1">Connection failed. Check your API key.</p>
            )}
          </div>
...
export function SettingsPanel({ open, onClose, settings, onSettingsChange }: SettingsPanelProps) {
  ...
  const updateProvider = (providerId: AgentProvider, updater: (p: ProviderConfig) => ProviderConfig) => {
    onSettingsChange({
      ...settings,
      providers: settings.providers.map((p) => (p.id === providerId ? updater(p) : p)),
    })
  }
                ...
                <div className="p-5 space-y-3">
                  {settings.providers.map((provider) => (
                    <ProviderCard
                      ...
                      onApiKeyChange={(key) =>
                        updateProvider(provider.id, (p) => ({ ...p, apiKey: key }))
                      }
                ...
                <div className="p-5 space-y-5">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <select
                        ...
                        onChange={(e) =>
                          onSettingsChange({ ...settings, defaultModel: e.target.value })
                        }
                        className="w-full appearance-none rounded-lg border border-border bg-background text-xs text-foreground px-3 py-2 pr-8 focus:outline-none focus:border-primary/40"
                      >
                        {settings.providers
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-[11px] text-muted-foreground mt-0.5">
                        Automatically create a PR when an agent completes its task
                      </p>
                    </div>
                    <Switch
