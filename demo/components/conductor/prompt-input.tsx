...
import { ArrowUp, Paperclip, AtSign, Slash, X, FileCode2, FileImage, FileText, File as FileIcon } from "lucide-react"
import { cn } from "@/lib/utils"
import { Button } from "@/components/ui/button"
import type { Attachment, AgentProvider } from "@/lib/store"

function fileTypeFromName(name: string): Attachment["type"] {
  const ext = name.split(".").pop()?.toLowerCase() ?? ""
  if (["png", "jpg", "jpeg", "gif", "svg", "webp"].includes(ext)) return "image"
  if (["ts", "tsx", "js", "jsx", "py", "rs", "go", "css", "html", "json", "sql"].includes(ext)) return "code"
      ...
      <div className="flex items-center justify-between mt-2 px-1">
        <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
          <span className="flex items-center gap-1">
            <kbd className="px-1 py-0.5 rounded bg-secondary text-[9px] font-mono border border-border">Enter</kbd>
            to send
          </span>
          <span className="flex items-center gap-1">
            <kbd className="px-1 py-0.5 rounded bg-secondary text-[9px] font-mono border border-border">Shift+Enter</kbd>
            for new line
          </span>
        </div>
        <span className={cn("text-[10px] font-mono", providerInfo.color)}>
          {providerInfo.label}
          {model && <span className="text-muted-foreground ml-1">/ {model.split("-").slice(0, 2).join("-")}</span>}
        </span>
      </div>
    </div>
  )
