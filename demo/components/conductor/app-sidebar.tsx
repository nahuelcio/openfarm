...
import type { Agent, AgentStatus, AgentProvider, Workspace } from "@/lib/store"

function StatusIcon({ status }: { status: AgentStatus }) {
  switch (status) {
    case "running":
      return <Loader2 className="h-3.5 w-3.5 text-agent-active animate-spin shrink-0" />
    case "completed":
      return <CheckCircle2 className="h-3.5 w-3.5 text-agent-active shrink-0" />
    case "reviewing":
...
function StatusLabel({ status }: { status: AgentStatus }) {
  ...
  return <span className={cn("text-[10px] font-medium uppercase tracking-wider", colors[status])}>{labels[status]}</span>
}

const PROVIDER_BADGE: Record<AgentProvider, { bg: string; text: string; label: string }> = {
  "claude-code": { bg: "bg-[#d97756]/15", text: "text-[#d97756]", label: "Claude" },
  codex: { bg: "bg-[#10a37f]/15", text: "text-[#10a37f]", label: "Codex" },
  opencode: { bg: "bg-[#06b6d4]/15", text: "text-[#06b6d4]", label: "OpenCode" },
}

function ProviderBadge({ provider }: { provider: AgentProvider }) {
  const p = PROVIDER_BADGE[provider]
  return (
    <span className={cn("text-[10px] font-mono px-1.5 py-0.5 rounded", p.bg, p.text)}>
      {p.label}
    </span>
  )
}
...
export function AppSidebar({ workspaces, selectedAgentId, onSelectAgent, onNewAgent }: AppSidebarProps) {
    ...
    <div className="flex h-full flex-col bg-sidebar">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-sidebar-border">
        <div className="flex items-center gap-2">
          <h2 className="text-sm font-semibold text-sidebar-accent-foreground">Workspaces</h2>
          {totalRunning > 0 && (
      ...
      <ScrollArea className="flex-1">
        <div className="py-1">
          {workspaces.map((workspace) => {
            ...
            const running = workspace.agents.filter((a) => a.status === "running").length
            return (
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            <div className="flex items-center gap-1 text-[11px] text-muted-foreground">
                              ...
                              <span className="truncate max-w-[120px]">{agent.branch}</span>
                            </div>
      ...
      <div className="border-t border-sidebar-border px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[12px] font-medium text-sidebar-accent-foreground truncate">John Doe</p>
